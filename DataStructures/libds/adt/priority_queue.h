...
namespace ds::adt {
    ...
    void TwoLists<P, T>::push(P priority, T data) {
        ...
        if (
            ...
            if (shortSequence_->size() == shortSequence_->getCapacity() &&
                ...
                LongSequenceBlockType* longSequnceBlock = &longSequence_->insertLast();

                longSequnceBlock->data_.priority_ = shortSequenceBlock->data_.priority_;
                longSequnceBlock->data_.data_ = shortSequenceBlock->data_.data_;
    ...
    T TwoLists<P, T>::pop() {
        ...
        if (shortSequence_->size() == 0 && longSequence_->size() > 0) {
            ...
            double sqrtResult = std::sqrt(originalLongSequence->size());
            double ceilResult = std::ceil(sqrtResult);
            shortSequence_->changeCapacity(static_cast<size_t>(ceilResult));
